...
// ─── Sweetgreen-inspired topping grid: 2-col, photography-first ───
const displayGroups = [
  ...
  { label: "Free add-ons", emoji: "✨", filter: (t) => t.g === "Free Add-ons" },
];
    ...
    <div>
      {active.length > 0 && (
        <div style={{ display: "flex", flexWrap: "wrap", gap: 5, marginBottom: 14 }}>
          {active.map(([n, q]) => (
            <span key={n} onClick={() => setOneTQ(n, 0)} style={{ fontSize: 13, background: RL, color: RUST, borderRadius: 14, padding: "5px 12px", cursor: "pointer", display: "inline-flex", alignItems: "center", gap: 4, fontWeight: 500, border: `1px solid ${RUST}` }}>
              {q === 2 ? `2× ${n}` : n} <span style={{ opacity: 0.5, fontSize: 14 }}>×</span>
            </span>
          ))}
        </div>
      )}
      {displayGroups.map(({ label, emoji, filter }) => {
        ...
        return (
          <div key={label} style={{ marginBottom: 16 }}>
            <p style={{ fontSize: 13, fontWeight: 600, color: MID, margin: "0 0 8px", display: "flex", alignItems: "center", gap: 6 }}><span>{emoji}</span> {label}</p>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
              {items.map((t) => {
                ...
                return (
                  <div key={t.n} onClick={cycle} style={{ position: "relative", borderRadius: 14, border: q > 0 ? `2px solid ${RUST}` : `1.5px solid ${BORDER}`, background: q > 0 ? RL : "#fff", cursor: "pointer", overflow: "hidden", transition: "all .15s", display: "flex", alignItems: "center", gap: 10, padding: "8px 10px" }}>
                    <div style={{ width: 52, height: 52, borderRadius: 10, background: WARM, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, overflow: "hidden" }}>
                      <img src={t.i} alt={t.n} style={{ width: 52, height: 52, objectFit: "cover" }} />
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <p style={{ fontSize: 13, fontWeight: q > 0 ? 600 : 500, color: q > 0 ? RUST : DARK, lineHeight: 1.25, marginBottom: 2 }}>{t.n}</p>
                      <p style={{ fontSize: 12, color: isFree ? GN : MID, fontWeight: isFree ? 600 : 400 }}>{isFree ? "FREE" : `+${price.toFixed(2)} ₾`}</p>
                    </div>
                    {q > 0 && <div style={{ position: "absolute", top: 6, right: 6, background: RUST, color: "#fff", borderRadius: 8, padding: "2px 7px", fontSize: 11, fontWeight: 700, letterSpacing: ".02em" }}>{q === 2 ? "2×" : "✓"}</div>}
                  </div>
                );
              })}
            </div>
    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 1000, background: "rgba(42,31,20,.6)", display: "flex", flexDirection: "column", justifyContent: "flex-end" }}>
      ...
      <div style={{ background: CREAM, borderRadius: "20px 20px 0 0", maxHeight: "92vh", display: "flex", flexDirection: "column" }}>
        ...
        <div ref={ref} style={{ flex: 1, overflowY: "auto" }}>
          ...
          <div style={{ width: "100%", height: 180, background: `linear-gradient(135deg, ${WARM} 0%, #EDE4D6 100%)`, display: "flex", alignItems: "center", justifyContent: "center", position: "relative" }}>
...
function PizzaCard({ pizza, onClick }) {
  ...
  return (
    <div onClick={onClick} style={{ background: "#fff", borderRadius: 16, overflow: "hidden", cursor: "pointer", border: `1px solid ${BORDER}`, transition: "transform .1s", WebkitTapHighlightColor: "transparent" }}>
      <div style={{ background: `linear-gradient(135deg, ${WARM} 0%, #EDE4D6 100%)`, height: 120, display: "flex", alignItems: "center", justifyContent: "center", position: "relative" }}>
        <img src={pizza.img} alt={pizza.name} style={{ width: 95, height: 95, borderRadius: "50%", objectFit: "cover", border: "2px solid #fff", boxShadow: "0 4px 16px rgba(42,31,20,.1)" }} />
      ...
      <div style={{ padding: "10px 12px 14px" }}>
        <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", marginBottom: 4 }}>
          <p style={{ fontFamily: "'Playfair Display',serif", fontSize: 16, fontWeight: 500, color: DARK }}>{pizza.name}</p>
          <p style={{ fontSize: 14, color: RUST, fontWeight: 600, flexShrink: 0 }}>{pizza.sizes[0].p.toFixed(0)} ₾</p>
        </div>
        <p style={{ fontSize: 12, color: MID, lineHeight: 1.35 }}>{ingPreview}</p>
      </div>
    </div>
  );
}
